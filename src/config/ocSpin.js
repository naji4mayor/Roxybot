...
export const SPIN_RARITY_WEIGHTS = {
    Common: 50,
    Uncommon: 22,
    Rare: 10,
    Epic: 8,
...
export function chooseSpinResult(type = 'race', config = {}) {
    ...
    const availableRarities = [...new Set(table.map(([rarity]) => rarity))];
    const totalWeight = availableRarities.reduce(
        (total, rarity) => total + (SPIN_RARITY_WEIGHTS[rarity] || 0),
        0,
    );
    let roll = Math.random() * totalWeight;
    let rarity = availableRarities[availableRarities.length - 1];

    for (const availableRarity of availableRarities) {
        roll -= SPIN_RARITY_WEIGHTS[availableRarity] || 0;
        if (roll < 0) {
            rarity = availableRarity;
            break;
        }
    }

    const results = table
        .filter(([entryRarity]) => entryRarity === rarity)
        .map(([, result]) => result);
