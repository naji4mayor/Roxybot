...
import { FRIENDLY_RESPONSES, TSUNDERE_GIFS, ROXY_PROFILE } from '../../events/messageCreate.js';
...
export default {
    ...
    execute: withErrorHandling(async (interaction) => {
        ...
        const fields = Object.entries(FRIENDLY_RESPONSES).map(([category, responses]) => ({
        ...
        fields.unshift({
            ...
            value: [
                `**Name:** ${ROXY_PROFILE.name}`,
                `**Role:** ${ROXY_PROFILE.role}`,
                `**Traits:** ${ROXY_PROFILE.traits.join(', ')}`,
                `**Likes:** ${ROXY_PROFILE.likes.join(', ')}`,
                `**Dislikes:** ${ROXY_PROFILE.dislikes.join(', ')}`,
                `**Habits:** ${ROXY_PROFILE.habits.join(', ')}`,
                `**Boundaries:** ${ROXY_PROFILE.boundaries.join('; ')}`,
            ].join('\n'),
            inline: false,
        });
