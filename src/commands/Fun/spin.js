...
export default {
    ...
    execute: withErrorHandling(async (interaction, config, client) => {
        ...
        const roleId = guildConfig.spinRoles?.[rarity.name];

        if (!roleId) {
            throw createError(
                'Spin role not configured',
                ErrorTypes.CONFIGURATION,
                `The **${rarity.name}** spin role has not been configured. Ask an administrator to use **/spin-config set**.`,
                { rarity: rarity.name },
            );
        }

        pendingSpins.push({ itemId: item.id, itemName: item.name, rarity: rarity.name, roleId });
