...
export default {
    data: new SlashCommandBuilder()
        ...
        .addSubcommand(subcommand => subcommand
            ...
            .setDescription('Remove a user\'s saved spin history')
            .addUserOption(option => option
                .setName('user')
                .setDescription('The user whose history should be removed')
                .setRequired(true))
            .addStringOption(option => option
                .setName('type')
                .setDescription('Category to remove, or all categories if omitted')
                .setRequired(false)
                .addChoices(...spinTypeChoices))),
    category: 'Fun',
    ...
    execute: withErrorHandling(async (interaction, config, client) => {
        ...
        if (subcommand === 'clear-history') {
            ...
            const remaining = type
                ? history.filter(spin => spin.spinType !== type)
                : [];
            const removedCount = history.length - remaining.length;

            userData.spinHistory = remaining;
            await setEconomyData(client, guildId, target.id, userData);
            const scope = type ? SPIN_TYPE_LABELS[type] : 'all spin categories';
            await InteractionHelper.safeEditReply(interaction, {
                embeds: [createEmbed({
                    title: 'Spin History Cleared',
                    description: `Removed **${removedCount}** result${removedCount === 1 ? '' : 's'} from **${scope}** for ${target}.`,
                    color: '#E67E22',
                })],
            });
            return;
